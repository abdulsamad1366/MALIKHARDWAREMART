...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const cartDoc = await Cart.findOne({ userId: authUser.userId })
      .populate({
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    // Verify authenticated user session and trade pricing approval
    const authUser = await getAuthUser(req);
    if (!authUser) {
      return NextResponse.json(
    ...
    if (authUser.role !== 'admin' && !authUser.isPriceVerified) {
      return NextResponse.json(
        { success: false, message: 'Your trade account is pending admin verification before you can add items or order.' },
        { status: 403 }
      );
    }
